...
| ADR-019 | Publish Saturn releases only from `saturn-vMAJOR.MINOR.PATCH` tags | The first production release needs a module-scoped immutable identity and must not reinterpret the historical `v0.0.1` CI tag as a Saturn production release | Release CI derives stable SemVer only from the exact Saturn prefix; Updater 0.4.0 resolves the same prefix; published bundles and OCI repositories use the Saturn name while legacy serialized/runtime identifiers remain compatible |

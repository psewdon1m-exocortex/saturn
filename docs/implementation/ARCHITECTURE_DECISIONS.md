...
| ADR-016 | Apply overwrite semantics when a file upload matches an active filename in the same folder | A duplicate name must have one deterministic outcome across the owner UI, Drop Point and API instead of failing or receiving an implicit suffix | The existing resource ID is retained, its previous bytes become a version, commits are serialized by folder and case-insensitive name, and a same-named folder remains a conflict |
| ADR-017 | Control dashboard transfers cooperatively at stream and chunk boundaries | The operator needs real pause, resume and cancellation without corrupting an object during verification or commit | Downloads pause their source stream; uploads and producer backups gate the next chunk or completion request; an owner upload interrupted by page refresh can reattach the unchanged source file and continue from the verified server offset; cancellation removes incomplete bytes; verifying and committing remain non-interruptible |
| ADR-018 | Implement previews and archives as Gateway capabilities, not browser or Storage Box extensions | A desktop Quick Look dependency would not provide a portable web security boundary, while direct archive access would bypass policy, audit and runtime storage switching | The web uses a Saturn Quick Preview modal over authenticated allowlisted Range streams; durable worker jobs create ZIP and extract ZIP/RAR through `FileService`, bounded local spool, validation and ordinary Saturn commit/trash semantics |
| ADR-019 | Publish Saturn releases only from `saturn-vMAJOR.MINOR.PATCH` tags | The first production release needs a module-scoped immutable identity and must not reinterpret the historical `v0.0.1` CI tag as a Saturn production release | Release CI derives stable SemVer only from the exact Saturn prefix; Updater 0.4.0 introduced support for that prefix and the current coordinated server-Nginx baseline requires 0.4.9+; published bundles and OCI repositories use the Saturn name while legacy serialized/runtime identifiers remain compatible |

...
function SettingsView({ preferences, setPreferences, addNotice, onAnonymous }: {
  ...
  const cards: Record<SettingsCardName, ReactNode> = {
    ...
    gryphon: <div className="settings-groups bot-connection-groups">
      ...
      <section className="settings-group"><h3>Gryphon version</h3><LocalAgentActions kind="gryphon" onComplete={loadGryphon} enabled={reauthed && updates?.updater.state === "ready"} /><p>Current installed version: <strong>{gryphon?.version ?? "unavailable"}</strong>{gryphonRelease?.available_version === undefined ? "" : ` · latest ${gryphonRelease.available_version}`}</p><button className="button settings-action" type="button" disabled={pending || gryphon === undefined} onClick={() => void checkGryphonUpdate()}>Check Gryphon for updates</button>{gryphonRelease?.update_available === true && gryphonRelease.available_version !== undefined ? <button className="button button--primary settings-action" type="button" disabled={pending} onClick={() => void installGryphonUpdate()}>Install Gryphon {gryphonRelease.available_version}</button> : null}</section>
    </div>,

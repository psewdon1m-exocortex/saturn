...
function SettingsView({ preferences, setPreferences, addNotice, onAnonymous }: {
  ...
  const cards: Record<SettingsCardName, ReactNode> = {
    ...
    backup: <div className="settings-groups backup-content">
      <HelperRecoveryPanel enabled={reauthed && updates?.updater.state === "ready"} />
  ...
  return <section className="workspace settings" aria-labelledby="settings-title">
    ...
    {updateDialog ? <Dialog title="Check for updates" description="Discovery is read-only. Installation creates a recovery snapshot and requires recent owner proof." onClose={() => setUpdateDialog(false)}><div className="update-discovery"><p>Installed: <strong className="accent-text">v{updates?.installedVersion ?? "unknown"}</strong></p><StatusRow label="Local updater agent" state={updates?.updater.state ?? "unavailable"} detail={updates?.updater.reason} /><StatusRow label="Approved registry" state={updates?.registry.state ?? "unavailable"} detail={updates?.registry.reason} />{updates?.discoveryEnabled ? <><LocalAgentActions kind="saturn" autoDiscover discoveryEnabled enabled={reauthed} onComplete={loadUpdates} />{reauthed ? null : <p className="muted">To install an available release, close this window and verify the current Access Key in Security → Recent owner proof.</p>}</> : <p className="muted">Discovery is unavailable until both trusted dependencies are configured. No update has been started.</p>}</div></Dialog> : null}

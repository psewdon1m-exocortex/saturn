...
export function App() {
  useEffect(() => {
    let active = true;
    void api.publicAppearance().then((appearance) => {
      if (active && /^#[0-9a-fA-F]{6}$/.test(appearance.accentColor)) document.documentElement.style.setProperty("--accent", appearance.accentColor);
    }).catch(() => undefined);
    return () => { active = false; };
  }, []);

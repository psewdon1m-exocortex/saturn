...
export function LocalAgentActions({ enabled, onComplete }: { readonly onComplete?: () => Promise<void>; readonly kind: "gryphon"; readonly enabled: boolean }) {
  const key = "exocortex.gryphon.job";
  ...
  return <div className="local-agent-actions">
    <button className="button" disabled={!enabled || busy} onClick={() => void run(api.initializeGryphon)}>Install or connect Gryphon</button>

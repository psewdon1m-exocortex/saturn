...
export async function resolveKernelOrigin(kernelUrl: string, token: string, service: "saturn" | "gryphon", timeoutMs: number, fetchImpl = fetch, management = false): Promise<string> {
  try {
    ...
    const managementKey = `services.${service}.management_url`;
    const keys = management ? [managementKey] : [hostKey, portKey];
    ...
    const result = data as { schema?: string; values?: Record<string, { value?: unknown; secret?: boolean }> };
    if (result.schema !== "exocortex.register.resolution.v1") throw new Error("Invalid Kernel response schema");
    if (management) {
      const record = result.values?.[managementKey];
      if (record?.secret !== false || typeof record.value !== "string" || record.value.length > 2048) throw new Error("Invalid management destination");
      const url = new URL(record.value);
      if (url.protocol !== "https:" || url.username || url.password) throw new Error("Invalid management destination");
      return url.href;
    }
...
export function registeredOrigin(database: Database, config: SaturnConfig, service: "saturn" | "gryphon", management = false) {
  return async (): Promise<string> => {
    ...
    return resolveKernelOrigin(kernel, (await fs.readFile(tokenFile, "utf8")).trim(), service, config.kernel.timeoutMs, fetch, management);
  };
}

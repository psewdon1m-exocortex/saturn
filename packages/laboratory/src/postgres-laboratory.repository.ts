...
interface AssetRow {
  id: string; resource_id: string; mode: LaboratoryAsset["mode"]; pinned_version_id: string | null; source_share_id: string | null; public_filename: string;
...
function asset(row: AssetRow): LaboratoryAsset {
  return { id: row.id, resourceId: row.resource_id, mode: row.mode,
    ...
    ...(row.source_share_id === null ? {} : { sourceShareId: row.source_share_id }),
...
export class PostgresLaboratoryRepository implements LaboratoryRepository {
  ...
  createAsset(value: LaboratoryAsset): Promise<LaboratoryAsset> { return this.database.withSql(async (sql) => { const rows=await sql<AssetRow[]>`INSERT INTO laboratory_assets(id,resource_id,mode,pinned_version_id,source_share_id,public_filename,label,disposition,state,created_at,updated_at) VALUES (${value.id},${value.resourceId},${value.mode},${value.pinnedVersionId??null},${value.sourceShareId??null},${value.publicFilename},${value.label},${value.disposition},${value.state},${value.createdAt},${value.updatedAt}) RETURNING *`;if(!rows[0])throw new Error("Laboratory asset create failed");return asset(rows[0]); }); }
  findActiveSharedAsset(resourceId: string, versionId: string, sourceShareId: string): Promise<LaboratoryAsset | undefined> { return this.database.withSql(async (sql) => { const rows=await sql<AssetRow[]>`SELECT * FROM laboratory_assets WHERE resource_id=${resourceId} AND pinned_version_id=${versionId} AND source_share_id=${sourceShareId} AND state='active' LIMIT 1`;return rows[0]===undefined?undefined:asset(rows[0]); }); }
